...
4. 再区分教学型字符设备读写例子和真实 function driver 的边界
5. 最后用示例驱动把初始化、收发、IRQ 接缝和退出路径串起来
...
### 3.3 从 API 到 CMD52/CMD53 再到 host request

`sdio_io.c` 这一层不能只记 API 名字，还要知道一次访问怎样变成真正的总线事务。

小寄存器访问通常走 CMD52：

```text
sdio_readb() / sdio_writeb()              // function driver 发起单字节寄存器读写
-> mmc_io_rw_direct()                     // SDIO core 把访问封装成 CMD52
-> mmc_wait_for_cmd()                     // MMC core 提交一个只有命令响应的请求
-> mmc_wait_for_req()                     // 进入通用 request 等待路径
-> host->ops->request(host, mrq)          // MMC core 和 host/controller driver 的分界点
-> sdhci_request()                        // SDHCI host 接管这个请求
-> sdhci_send_command()                   // 控制器真正把 CMD52 发到 SDIO 总线上
-> cmd.resp[] / R5                        // card 返回 R5，core 解析错误位和读回值
```

批量数据传输通常走 CMD53：

```text
sdio_memcpy_fromio() / sdio_memcpy_toio() // function driver 发起 FIFO/window/buffer 传输
-> sdio_io_rw_ext_helper()                // SDIO core 处理地址递增、block/byte mode 和分段
-> mmc_io_rw_extended()                   // SDIO core 把访问封装成 CMD53
-> mmc_wait_for_req()                     // MMC core 提交带 data 阶段的 request
-> host->ops->request(host, mrq)          // 请求进入 host/controller driver
-> sdhci_request()                        // SDHCI host 处理命令、数据和完成事件
-> data complete / error                  // 控制器中断或轮询完成后，core 返回传输结果
```

这两条链的边界非常关键：

- function driver 看到的是 `sdio_readb()`、`sdio_writeb()`、`sdio_memcpy_fromio()`、`sdio_memcpy_toio()`
- SDIO core 负责把它们翻译成 CMD52 或 CMD53
- MMC core 负责把请求统一交给 `host->ops->request`
- host/controller driver 负责真正发命令、搬数据、等完成和回填 response

所以排查 I/O 问题时，不能只说“读寄存器失败”或“搬数据失败”，还要继续问：失败发生在 API 参数、SDIO 命令封装、host request、控制器完成，还是 response/data 解析阶段。
...
## 8. 教学型字符设备读写例子 vs 真实 function driver

一些入门文章会用字符设备方式演示 SDIO API：注册一个 `miscdevice` 或 `cdev`，再在 `file_operations.read/write/ioctl` 里调用 `sdio_readb()`、`sdio_writeb()` 或 `sdio_memcpy_fromio()/toio()`。

这种写法适合教学，因为它把用户态读写和 SDIO API 直接连起来，方便观察 CMD52/CMD53 的效果。但它不是大多数真实 WiFi/BT SDIO 驱动的产品形态。

教学型字符设备模型可以这样理解：

```text
用户态 read()/write()/ioctl()              // 为了教学或调试，人为暴露一个字符设备入口
-> file_operations.read/write/ioctl        // 字符设备层把用户请求交给驱动
-> copy_from_user() / copy_to_user()       // 在用户缓冲区和内核缓冲区之间搬数据
-> sdio_claim_host()                       // 独占 SDIO host，保护后续总线访问
-> sdio_readb()/writeb()                   // 小寄存器访问，通常封装成 CMD52
-> sdio_memcpy_fromio()/toio()             // 批量数据访问，通常封装成 CMD53
-> sdio_release_host()                     // 释放 host 访问权
```

真实 WiFi/BT function driver 通常不是这样直接暴露 `/dev/xxx` 给用户读写，而是接入对应的内核子系统：

```text
网络栈 / 蓝牙栈 / 其他上层子系统              // 真实业务入口通常来自内核子系统，而不是用户直接 read/write
-> WiFi/BT driver 的 tx/rx/control 路径      // 芯片私有驱动处理协议、队列、固件和状态机
-> driver 私有 bus ops 或 transport 封装     // 把上层请求转换成 SDIO 读写、中断和锁操作
-> sdio_claim_host()                       // 进入 SDIO 总线访问前仍然需要 host 访问权
-> sdio_readb/writeb 或 memcpy_fromio/toio  // 底层仍然落到标准 SDIO API
-> CMD52 / CMD53 / host->ops->request      // 最终还是进入 MMC/SDIO core 和 host 控制器
-> sdio_release_host()                     // 当前 SDIO 事务结束
```

所以这类教学例子的价值是：

- 用最短路径练习 `sdio_*` API
- 方便验证 CMD52/CMD53、block size、claim host 和返回值
- 适合做 bring-up 早期的临时调试入口

但它的边界也要明确：

- 它不能代表真实 WiFi 驱动的完整结构
- 它通常不处理 mac80211/cfg80211、蓝牙 HCI、固件状态机和复杂队列
- 它容易让人误以为 SDIO function driver 必须暴露字符设备，实际并不是

因此，学习时可以把字符设备例子当作“API 放大镜”，而把 `wfx` 这类驱动当作“真实工程形态”。前者回答“这个 API 怎么调”，后者回答“真实芯片驱动如何把这些 API 放到子系统里工作”。
...
- 第 `6` 节：初始化顺序
- 第 `7` 节：块大小的重要性
- 第 `9` 节：这些规则在真实驱动里的完整落点

## 10. 全流程总结：从 function driver API 到 host 控制器

这一章的主线可以按“驱动 API -> SDIO 命令 -> MMC request -> host ops -> 硬件完成”来复盘。

```text
driver probe/work/tx/rx path              // 具体 function driver 的初始化或收发上下文
-> sdio_claim_host(func)                  // 独占当前 mmc_host，避免并发命令互相打断
-> sdio_enable_func(func)                 // 通过 CMD52 打开当前 function 的使能位
-> sdio_set_block_size(func, size)        // 配置 function 的 block size，影响后续 CMD53
-> sdio_readb() / sdio_writeb()           // 小寄存器访问，通常封装成 CMD52
-> sdio_memcpy_fromio() / toio()          // 批量数据访问，通常封装成 CMD53
-> mmc_wait_for_req()                     // MMC core 把命令/数据请求提交给 host
-> host->ops->request(host, mrq)          // 从通用 core 进入具体 host/controller driver
-> sdhci_request() / sdhci_send_command() // SDHCI 控制器发命令、搬数据、等待完成
-> sdio_release_host(func)                // 当前访问结束，释放 host 使用权
```

这条线和前面章节的关系是：`03` 章负责把具体 driver probe 拉起来，本章说明 probe 之后 driver 怎样通过标准 SDIO API 访问芯片；再往后 `05` 章则说明芯片反向通知 driver 时，IRQ 怎样回到 function handler。
