...
host 控制器注册 -> SDIO 卡识别 -> 整卡初始化 -> 创建 function -> 注册到 sdio bus -> 匹配 `sdio_driver` -> 进入 function driver probe -> 建立 I/O 与 IRQ 通路
...
- `06` 先把标准 SDIO function driver 的最小顺序站稳
  - `probe -> enable_func -> set_block_size -> I/O -> IRQ -> remove`
...
```mermaid
flowchart TD
    A["host 驱动注册 mmc_host"] --> B["检测到卡"]
    B --> C["mmc_attach_sdio()"]
    C --> D["mmc_sdio_init_card()"]
    D --> E["sdio_read_cccr() / sdio_read_common_cis()"]
    D --> F["sdio_init_func() 为每个 function 创建 struct sdio_func"]
    F --> G["mmc_add_card()"]
    G --> H["sdio_add_func()"]
    H --> I["sdio_bus_probe()"]
    I --> J["具体 sdio_driver.probe()"]
```
...
- `检测到卡`
  - 作用：让 MMC core 开始一次卡识别流程
  - 含义：后续才会判断当前插入的是 MMC、SD 还是 SDIO
- `mmc_attach_sdio()`
