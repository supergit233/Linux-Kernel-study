...
```text
gyro driver probe                 // 陀螺仪驱动绑定硬件，准备采样
...
```mermaid
flowchart TD
    A["gyro hardware"] --> B["gyro driver"]
    B --> C["gyro samples + timestamps"]
...
这张图要表达的是：陀螺仪防抖不是“gyro 数据一来就直接旋转画面”，而是必须经过时间同步和坐标系标定，才能把传感器运动正确映射到图像补偿。
